...
*/
	...
	roiManager("Add");
	roiManager("Select", roiManager("count")-1);
	roiManager("Rename", "Shoulder_2");
	roiManager("deselect"); 
	roiManager("show none");

	// Create FC region
	
	Array_With_ROIs_Indexes =  newArray(2,7);
	roiManager("Select",Array_With_ROIs_Indexes);
	roiManager("XOR");
	roiManager("Add");
	roiManager("Select", roiManager("count")-1);
	roiManager("Rename", "Plaque-Shoulder_1");
	Array_With_ROIs_Indexes =  newArray(8,9);
	roiManager("Select",Array_With_ROIs_Indexes);
	roiManager("XOR");
	roiManager("Add");
	roiManager("Select", roiManager("count")-1);
	roiManager("Rename", "Plaque-NO_Shoulders");
	Array_With_ROIs_Indexes =  newArray(1,10);
	roiManager("Select",Array_With_ROIs_Indexes);
	roiManager("AND");
	roiManager("Add");
	roiManager("Select", roiManager("count")-1);
	...
	Array_With_ROIs_Indexes =  newArray(2,7,8,11);
	roiManager("Select",Array_With_ROIs_Indexes);
	roiManager("XOR");
	roiManager("Add");
	roiManager("Select", roiManager("count")-1);
	roiManager("Rename", "Core region");
	roiManager("deselect"); 
	roiManager("show none");
...
//Adjust contrast in original
	...
	selectImage(myORIImage);
	EnhanceChannelsContrast(myORIImage,1,300,1500);
	...
	myroisarray=newArray(2,7,8,11,12);
	myroiscolors=newArray("red","green","blue","yellow","magenta");
